...
export const recordsSlice = createSlice({
  ...
  reducers: {
    ...
    removeRecord(state, action: PayloadAction<TabRecord>) {
      const index = state.records.findIndex(({id}) => id === action.payload.id)
      if (index === -1) return
      state.records.splice(index, 1)
      if (state.focus?.id === action.payload.id) state.focus = undefined
    },
    setFocus(state, action: PayloadAction<TabRecord | undefined>) {
